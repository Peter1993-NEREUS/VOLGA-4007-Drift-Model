...
def read_uv(path, depth_mean=False):
    with Dataset(path) as ds:
        ...
        u=np.asarray(ds.variables['uo'][:],dtype=np.float32)
        v=np.asarray(ds.variables['vo'][:],dtype=np.float32)
        if u.ndim==4:
            if depth_mean:
...
def read_stokes(path):
    with Dataset(path) as ds:
        lat=np.asarray(ds.variables['latitude'][:],dtype=np.float32)
        lon=np.asarray(ds.variables['longitude'][:],dtype=np.float32)
        u=np.asarray(ds.variables['VSDX'][:],dtype=np.float32)
        v=np.asarray(ds.variables['VSDY'][:],dtype=np.float32)
        tvar=ds.variables['time']
        times=num2date(tvar[:],tvar.units,getattr(tvar,'calendar','standard'),only_use_cftime_datetimes=False)
    return times,lat,lon,u,v
...
def main():
    ...
    meta={'format':'VOLGA_DRIFTPACK_V1_1_FULLLOAD_MOBILE','scale':SCALE,'missing':MISSING,'startUtc':'2026-08-19T07:00:00Z','endUtc':'2026-09-01T07:00:00Z','localOffsetHours':3,'fixed':{'vessel':'VOLGA-4007','imo':'8728816','cargo':'6,000 MT COPPER','draftM':4.68,'leeway':0.003,'integrationMinutes':15,'blendHours':24,'loadCondition':'FULL LOAD / SUMMER DRAFT','currentVerticalMean':'CMEMS available levels 0.50-4.52 m','leewayNote':'Fixed engineering assumption for full-load v1.1; k=0.003 (0.3% of wind speed downwind)'},'regional':reg,'stokes':stk,'global':glo,'domain':{'minLat':42.5,'maxLat':45.5,'minLon':35.0,'maxLon':39.5}}
